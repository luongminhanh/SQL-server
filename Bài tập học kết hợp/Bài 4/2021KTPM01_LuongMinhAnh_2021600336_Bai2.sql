...
select top 10 Xuat.SoHDX, NgayXuat, SoLuongX from PXuat
...
select SanPham.MaSP, TenSP from Xuat
inner join SanPham on SanPham.MaSP = Xuat.MaSP
inner join HangSX on HangSX.MaHangSX = SanPham.MaHangSX
...
select TenPhong, count(MaNV) as 'SLNhanVien' from NhanVien
where GioiTinh = 'Nam'
group by TenPhong

--e. Thống kê tổng số lượng nhập của mỗi hãng sản xuất trong năm 2018.
select TenHang, SUM(SoLuongN) as 'TongSLNhap' from Nhap
inner join SanPham on SanPham.MaSP = Nhap.MaSP
inner join HangSX on HangSX.MaHangSX = SanPham.MaHangSX
inner join PNhap on PNhap.SoHDN = Nhap.SoHDN
...
--f. Hãy thống kê xem tổng lượng tiền xuất của mỗi nhân viên trong năm 2018 là bao nhiêu.select NhanVien.MaNV, TenNV, SUM(SoLuongX*GiaBan) as 'TongTienX' from NhanVien
inner join SanPham on NhanVien.MaNV = SanPham.MaSP
